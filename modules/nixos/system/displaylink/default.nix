{ lib, config, pkgs, inputs, namespace, ... }:
...
in
{
  ...
  config = mkIf cfg.enable (let
    ...
    unstablePkgsForSystem = lib.attrByPath ["unstable" "legacyPackages" config.nixpkgs.system] null inputs;
    ...
    customDisplaylinkPackage =
      if evdiBuiltForSystemKernel != null then
        (pkgs.displaylink.override { # IMPORTANT: Use displaylink from your system's `pkgs`
          evdi = evdiBuiltForSystemKernel;
          requireFile = _: pkgs.fetchurl {
    ...
    assertions = [
      {
        ...
        message = ''
          ...
          - inputs.unstable ? "legacyPackages" (if inputs.unstable exists): ${if inputs ? "unstable" then toString (inputs.unstable ? "legacyPackages") else "N/A"}
          - Type of inputs.unstable.legacyPackages (if exists): ${if inputs ? "unstable" && inputs.unstable ? "legacyPackages" then builtins.typeOf inputs.unstable.legacyPackages else "N/A"}
          - inputs.unstable.legacyPackages ? "${config.nixpkgs.system}" (if parent attrs exist and are attrsets): ${
              ...
              in
              if builtins.isAttrs lp then toString (lp ? config.nixpkgs.system) else "N/A (inputs.unstable.legacyPackages not an attrset or not found)"
            }
