...
in
{
  options.${namespace}.services.k8s = with types; {
    ...
    tokenFile = mkOpt str "" "File path containing k3s token to use when connecting to the server. This option only makes sense for an agent.";
    serverAddr = mkOpt str "" "Address of the server node to connect to (not needed for the first node).";
    role = mkOpt (enum [ "server" "agent" ]) "agent" "Role of this node (server or agent).";
    # Network type - mutually exclusive options
    networkType = mkOpt (enum ["standard" "tailscale" "wireguard" "cilium"]) "standard" "Network type to use for cluster communications.";
  ...
  config = mkIf cfg.enable {
    ...
    system.activationScripts = mkIf (cfg.networkType == "cilium") {
      install-cilium = let
        ciliumInstallScript = if cfg.cilium.installScript != "" then cfg.cilium.installScript else ''
          #!/bin/sh
