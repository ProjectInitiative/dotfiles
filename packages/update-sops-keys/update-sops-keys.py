...
def get_host_age_pubkey(host, user, ssh_key_path):
    ...
    try:
        ...
        age_pubkey_process = subprocess.run(
            ["ssh-to-age"],
            input=ssh_pubkey,
            capture_output=True,
            text=True,
        ...
        return age_pubkey_process.stdout.strip()
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f"Failed to retrieve or convert host key: {e.stderr}")
...
def reencrypt_secrets(secrets_file):
    try:
        subprocess.run(["sops", "updatekeys", secrets_file], check=True)
    except subprocess.CalledProcessError as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--secrets-file", default="secrets.yaml", help="Path to secrets file to re-encrypt")
    ...
    try:
        ...
        if updated:
            ...
            reencrypt_secrets(args.secrets_file)
